...
def init_hidden(x: torch.Tensor, hidden_size: int, num_dir: int = 1, xavier: bool = True):
    ...
    Args:
        ...
        xavier: (bool): whether use xavier initialization or not
    """
    if xavier:
...
class Encoder(nn.Module):
    def __init__(self, config, input_size: int):
        ...
        self.fc = nn.Linear(config['hidden_size_encoder'], 1)
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            input_encoded[:, t, :] = h_t
        latent = self.fc(h_t.squeeze(0))  # [batch_size, 1]
        return _, latent
...
class AttnEncoder(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            input_encoded[:, t, :] = h_t
            attentions[:, t, :] = a_t
        latent = self.fc(h_t.squeeze(0))
        return attentions, latent
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        # Capa para expandir el espacio latente 1D
        self.expand = nn.Linear(1, self.hidden_size)  # 1D -> hidden_size
    ...
    def forward(self, _, y_hist: torch.Tensor):
        ...
        """
        # Expandir el espacio latente 1D
        expanded = self.expand(y_hist).unsqueeze(1).repeat(1, self.seq_len, 1)  # [batch, seq_len, encoder_hidden_size]
        h_t, c_t = (init_hidden(expanded, self.hidden_size),
                    init_hidden(expanded, self.hidden_size))
...
class AttnDecoder(nn.Module):
    def __init__(self, config):
        ...
        # Capa para expandir el espacio latente 1D
        self.expand = nn.Linear(1, self.encoder_hidden_size)  # 1D -> hidden_size
    ...
    def forward(self, input_encoded: torch.Tensor, y_history: torch.Tensor):
        ...
        """
        expanded = self.expand(input_encoded).unsqueeze(1).repeat(1, self.seq_len, 1)  # [batch, seq_len, encoder_hidden_size]
        h_t, c_t = (
            init_hidden(expanded, self.decoder_hidden_size), init_hidden(expanded, self.decoder_hidden_size))
        context = Variable(torch.zeros(expanded.size(0), self.encoder_hidden_size))

        for t in range(self.seq_len):
            x = torch.cat((h_t.repeat(self.seq_len, 1, 1).permute(1, 0, 2),
                           c_t.repeat(self.seq_len, 1, 1).permute(1, 0, 2),
                           expanded.to(device)), dim=2)
            ...
            context = torch.bmm(x.unsqueeze(1), expanded.to(device))[:, 0, :]  # (batch_size, encoder_hidden_size)
...
class AutoEncForecast(nn.Module):
    def __init__(self, config, input_size, get_latent=False, decode_latent=False, ):
        """
        Initialize the network.
        ...
        self.decode_latent = decode_latent

    def encode(self, x: torch.Tensor) -> tuple[Any, Any]:
        """Extrae el espacio latente (sin decodificar)"""
        attentions, latent = self.encoder(x)
        return attentions, latent  # [batch_size, seq_len, hidden_size] o [batch_size, 1] según tu encoder

    def decode(self, latent: torch.Tensor, y_hist: torch.Tensor) -> torch.Tensor:
        """Decodifica desde el espacio latente"""
        return self.decoder(latent, y_hist.float())
...
#        outputs = self.decoder(encoder_output, y_hist.float())
        attentions, latent = self.encode(encoder_input)
        outputs = self.decode(latent, y_hist)
